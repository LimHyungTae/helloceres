...
<p><strong><em>Graph SLAM을 위한 Ceres Solver 단계별 한국어 튜토리얼.</em></strong></p>

</div>

원저자: Hyungtae Lim (shapelim@kaist.ac.kr)
...
> **macOS 주의 (Anaconda 사용자):** Homebrew와 Anaconda가 함께 설치된 경우, gflags/glog 충돌을 방지하기 위해 Homebrew 의존성 경로를 아래와 같이 명시적으로 지정하십시오.
> ```bash
> cmake -B build -S . -DCMAKE_BUILD_TYPE=Release \
...
```bash
bash scripts/run_tests.sh build   # macOS의 경우 build_mac 사용
```
